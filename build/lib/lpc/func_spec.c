 ...
 *
 * LPC operator and efun specification for the Neolith driver.
 * Processed in three passes during the build:
 *
 *   Pass 1 (CMake configure): func_spec.c.in  →  func_spec.c
 *     cmake configure_file() substitutes  placeholders.
 *
 *   Pass 2 (C preprocessor): func_spec.c  →  func_spec.i
 *     Expands #ifdef / #define guards from <config.h> so conditional
 *     efuns (e.g. PACKAGE_SOCKETS, DISALLOW_BUFFER_TYPE) are resolved.
 *
 *   Pass 3 (edit_source POST_BUILD): func_spec.i  →  efuns_*.h
 *     Bison parser in make_func.y reads the preprocessed spec and emits
 *     the opcode, prototype, vector, and definition headers.
 *
 * Configuration: feature guards come from <config.h> (CMake-generated).
 * The MudOS local_options file is no longer used.
 *
 * WARNING: Adding, removing, or reordering operators or efuns changes opcode
 * assignments and breaks binary compatibility with saved LPC programs.
 * Bump driver_id in lib/lpc/program/binaries.c whenever this file,
 * config.h settings, or options.h definitions change in a way that
 * alters the generated opcode table.
 */
#ifdef	HAVE_CONFIG_H
#define _LPC_FUNC_SPEC
#include <config.h>
#endif	/* HAVE_CONFIG_H */
 ...
 * CURL REST API efuns
 */
void perform_using(string, mixed);
...
/* ============================================================
 * LJ-FFI Dynamic FFI Efuns (LuaJIT-compatible)
 * ============================================================ */

/* Parse C declarations and register types/functions */
int ffi_cdef(string);
...
/* Get sizeof a registered type */
int ffi_sizeof(string);

/* Bind a symbol name to a C function pointer (via dlsym) */
...
/* Spatial index: create(config_mapping) */
int spatial_create(mapping);

/* Spatial index: destroy(handle) */
void spatial_destroy(int);

/* Spatial index: insert(handle, entity_id, pos_array{x,y,z}) */
int spatial_insert(int, int, mixed);

/* Spatial index: remove(handle, entity_id) */
int spatial_remove(int, int);

/* Spatial index: move(handle, entity_id, pos_array{x,y,z}) */
int spatial_move(int, int, mixed);

/* Spatial index: get_pos(handle, entity_id) -> {x,y,z} or 0 */
mixed spatial_get_pos(int, int);

/* Spatial index: query_radius(handle, x, y, radius) -> int[] */
mixed spatial_query_radius(int, float, float, float);

/* Spatial index: query_rect(handle, rect_mapping) -> int[] */
mixed spatial_query_rect(int, mapping);

/* Spatial index: stats(handle) -> mapping */
mapping spatial_stats(int);

/* AOI: create(spatial_handle, view_radius) */
int aoi_create(int, float);

/* AOI: destroy(handle) */
void aoi_destroy(int);

/* AOI: subscribe(handle, entity_id) */
int aoi_subscribe(int, int);

/* AOI: unsubscribe(handle, entity_id) */
int aoi_unsubscribe(int, int);

/* AOI: get_visible(handle, entity_id) -> int[] */
mixed aoi_get_visible(int, int);

/* AOI: get_delta(handle, entity_id) -> mapping */
